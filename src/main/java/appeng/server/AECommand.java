...
public final class AECommand extends CommandBase {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] ss) {
        ...
        if (ss.length == 0 || ss.length == 1
                && (test.isEmpty() || Arrays.stream(Commands.values()).anyMatch(c -> c.toString().startsWith(test))))
            Arrays.stream(Commands.values()).map(Commands::toString).filter(c -> test.isEmpty() || c.startsWith(test))
                    .forEach(l::add);
        return l;
    }
