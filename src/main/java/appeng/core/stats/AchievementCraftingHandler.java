...
/**
 * handles the achievement when an {@link net.minecraft.item.Item} is crafted by a player. The achievement is only added
 * if its a real {@link net.minecraft.entity.player.EntityPlayer}.
 *
 * @author thatsIch
 * @since rv2
 */
        ...
        for (final Achievements achievement : Achievements.values()) {
            switch (achievement.getType()) {
                case Craft:
                    ...
                    break;
                case CraftItem:
                    if (achievement.getStack() != null
                            && achievement.getStack().getItem().getClass() == event.crafting.getItem().getClass()) {
                        achievement.addToPlayer(event.player);
                        return;
                    }
                default:
