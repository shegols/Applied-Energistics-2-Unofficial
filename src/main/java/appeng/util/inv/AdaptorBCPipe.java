...
public class AdaptorBCPipe extends InventoryAdaptor {
    ...
    public AdaptorBCPipe(final TileEntity s, final ForgeDirection dd) {
        this.buildCraft =
                (IBuildCraftTransport) IntegrationRegistry.INSTANCE.getInstance(IntegrationType.BuildCraftTransport);
    ...
    public ItemStack addItems(final ItemStack toBeAdded) {
        ...
        if (IntegrationRegistry.INSTANCE.isEnabled(IntegrationType.BuildCraftTransport)
                && this.buildCraft.addItemsToPipe(this.i, toBeAdded, this.d)) {
            return null;
        }
        return toBeAdded;
    }
